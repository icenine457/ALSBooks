...
exports.list = function (req, res) {
  ...
  _.extend(options.criteria, {archived: {$exists: false}});

  // TODO: Rewrite using Q
    ...
    .exec(function(err, user) {
      ...
      if (user) {
        hasAbility = _.chain(user.abilities)
          .pluck('title')
          .contains('canEditPublications')
          .value()
      }

      options.criteria.verified = true;
      if (hasAbility) {
        delete options.criteria["verified"]
      }
