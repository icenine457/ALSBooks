  ...
  User.findOne({_id: req.user._id})
    ...
    .exec(function(err, user) {
      ...
      if (req.route.method == "get" && ! _.contains(match, 'read')) {
        console.error("Unauthorized attempt to GET from a route.")
        return res.send(401, 'Unauthorized');
      }


      if ((req.route.method == "post" || req.route.method == "put") && ! _.contains(match, 'write')) {
        console.error("Unauthorized attempt to POST to a route.")
        return res.send(401, 'Unauthorized');
      }

      if ((req.route.method == "delete") && ! _.contains(match, req.route.method )) {
        console.error("Unauthorized attempt to DELETE to a route.")
        return res.send(401, 'Unauthorized');
      }

      if ( ( req.url.indexOf("edit") !== -1 || req.url.indexOf("new") !== -1 ) && ! _.contains(match, 'write') ) {
        console.error("Unauthorized attempt to edit or create a new entity.")
        return res.send(401, 'Unauthorized');
      }
      next();
  });
